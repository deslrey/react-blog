...
import { useVisitorToken } from '@/app/hooks/useVisitorToken';
...
const BlogDetails: React.FC<BlogDetailsProps> = ({ params }) => {
    ...
    const [hasVisitorToken, setHasVisitorToken] = useState<boolean>(false);
    ...
    useEffect(() => {
        ...
        if (!token) {
            // 触发 hook逻辑
            useVisitorToken("文章详情");
        }
        setHasVisitorToken(!!token);
    }, []);
    ...
    useEffect(() => {
        ...
        const loadMarkdown = async () => {
            try {
                ...
                // 解析 YAML Front Matter 分割内容
                const match = text.match(/^---\r?\n([\s\S]+?)\r?\n---\r?\n([\s\S]*)$/);
                if (match) {
                    setMarkdownContent(match[2]);
                    setHeadings(extractHeadings(match[2]));
                } else {
                    console.warn('YAML 匹配失败，原文:', text.slice(0, 300));
